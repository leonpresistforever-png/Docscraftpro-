...
export function LandingPaperDraft() {
  ...
  useEffect(() => {
    let timer: any = null;
    const targetWords = DRAFT_TEMPLATES[activeTemplateIdx].text.split(' ');
    ...
    timer = setInterval(() => {
      if (wordIdx < targetWords.length) {
        setDisplayedWords(prev => [...prev, targetWords[wordIdx]]);
        wordIdx++;
      } else {
        clearInterval(timer);
        setIsTyping(false);
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(DRAFT_TEMPLATES[activeTemplateIdx].text);
    setCopied(true);
