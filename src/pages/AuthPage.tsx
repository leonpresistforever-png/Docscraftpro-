...
export function AuthPage() {
  const { user, signInWithGoogle, signInWithApple, signInWithEmail, signUpWithEmail, signInAsDemo } = useAuth();
  ...
  const handleAppleAuth = async () => {
    ...
    const cvVerified = isCustomCaptchaVerified;
    if (!cfVerified && !cvVerified) {
      setError('Please verify the Captcha to log in with Apple.');
      return;
    }
    try {
      if (captchaToken) sessionStorage.setItem('temp_cf_verified', 'true');
      ...
      if (error && typeof error === 'object' && (error.code === 'auth/operation-not-allowed' || error.message?.includes('operation-not-allowed'))) {
        setError("Apple Sign-In is not fully activated in this Firebase Console project. Please enable the Apple identity provider in your Firebase Authentication Settings, or use Google or standard Email sign-in for testing.");
      } else {
        setError(error.message || "Failed to authenticate with Apple. Please try again.");
      }
    }
  };
            ...
            <div className={`absolute inset-0 w-full [backface-visibility:hidden] flex flex-col items-center bg-white/80 backdrop-blur-xl border border-white p-8 rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] overflow-y-auto`}>
              ...
              <div className="flex flex-col gap-2.5 w-full">
                ...
                <button 
                  ...
                  className="w-full h-12 bg-black text-white rounded-xl flex items-center justify-center gap-3 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 text-sm font-bold"
                >
