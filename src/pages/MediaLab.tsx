...
import { Music, Video, Image as ImageIcon, Volume2 } from 'lucide-react';
import { generateImage, textToSpeech, generateMusic, generateVideo } from '../lib/gemini';
  ...
  const handleGenerate = async () => {
    ...
    try {
      if (tab === 'image') {
        ...
        // Just demonstrating TTS here, but could swap logic for Music by checking a toggle
        ...
        if (isMusic) {
         ...
         // Video gen can take a while and result in an operation
         alert("Video generation started. This takes several minutes using Veo. Check console for op id.");
         generateVideo(prompt, aspectRatio as any).then(op => console.log('Video op started:', op));
      }
    } catch (err: any) {
      ...
      <div className="flex-1 flex flex-col pt-8">
        ...
        <div className="flex-1 px-12 py-8 grid grid-cols-1 lg:grid-cols-4 gap-12 overflow-y-auto">
            ...
            {tab === 'image' && (
              <>
                <div>
                  ...
                  <select className="w-full border border-dc-border rounded-md p-2" value={aspectRatio} onChange={(e) => setAspectRatio(e.target.value as any)}>
                ...
                <div>
                  ...
                  <select className="w-full border border-dc-border rounded-md p-2" value={imageSize} onChange={(e) => setImageSize(e.target.value as any)}>
            ...
            {tab === 'video' && (
              <div>
                <label className="block text-sm font-bold mb-2">Aspect Ratio</label>
                  ...
                  <p className="text-xs text-amber-600 mt-2">Note: Video generation with Veo takes up to ~3 minutes.</p>
              </div>
            )}

            {tab === 'audio' && (
               <div>
                  <p className="text-xs text-dc-text-muted mt-2">Include the word "music" or "song" in your prompt to use Lyria generation, otherwise it uses TTS.</p>
               </div>
            )}
          ...
          <div className="lg:col-span-3 border border-dc-border bg-[#F5F5F3] rounded-2xl flex items-center justify-center overflow-hidden min-h-[600px] shadow-inner">
            {!resultImage && !resultAudio && !loading && (
            ...
            {loading && (
              <div className="flex flex-col items-center animate-pulse">
                <div className="w-12 h-12 border-4 border-dc-gold border-t-transparent rounded-full animate-spin mb-4"></div>
                <p>Crafting your media...</p>
              </div>
            )}
            ...
            {resultAudio && !loading && tab === 'audio' && (
              <div className="p-8 bg-white rounded-xl shadow-sm rounded-full">
